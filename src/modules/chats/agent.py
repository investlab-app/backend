...
def create_agent() -> Agent[AgentDeps, str]:
    ...
    async def get_total_value(ctx: RunContext[AgentDeps]) -> str:
        ...
        total_value = await sync_to_async(InvestorStatsService().get_total_value)(
            investor
        )
    ...
    async def get_asset_allocation(ctx: RunContext[AgentDeps]) -> str:
        ...
        """
        investor = await Investor.objects.aget(id=ctx.deps.investor_id)
        asset_allocation = await sync_to_async(
            InvestorStatsService().get_asset_allocation
        )(investor)
    ...
    async def get_stats(ctx: RunContext[AgentDeps]) -> str:
        ...
        def _compute_stats():
            ...
            return MultiInstrumentsTransactionStatsService(investor).compute_stats()
