...
async def get_portfolio(ctx: RunContext[str]) -> dict:
    """
    ...
    try:
        ...
        if result.get("status") == "success":
            logger.info(f"Successfully fetched portfolio for investor {investor_id}")
        return result
    except Investor.DoesNotExist:
        logger.warning(f"Investor not found: {investor_id}")
        return {"status": "error", "message": "Investor not found"}
    except Exception as e:
        logger.exception(f"Error fetching portfolio for investor {investor_id}: {e}")
        return {"status": "error", "message": str(e)}


async def get_transactions(ctx: RunContext[str], limit: int = 20) -> dict:
    """
    ...
    investor_id = ctx.deps
    try:
        logger.info(f"Fetching transactions for investor {investor_id}, limit={limit}")
        ...
        logger.exception(f"Error fetching transactions for investor {investor_id}: {e}")
        return {"status": "error", "message": str(e)}


async def get_portfolio_performance(
    ctx: RunContext[str], period_days: int = 30
) -> dict:
    """
