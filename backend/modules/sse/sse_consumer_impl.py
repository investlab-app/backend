...
class SSEConsumerImpl(SSEConsumer):
    ...
    async def live_prices_handler(self, prices: dict[str, any]) -> None:
        ...
        filtered_prices = {symbol: price for symbol, price in prices.items() if symbol in client_symbols}
    ...
    async def handle(self, params):
        ...
        self.log.debug(f"{self.connection_id}: Starting SSE stream with symbols: {symbols}")
        ...
        try:
            ...
            self.log.error(f"{self.connection_id}: An error occurred in the stream: {e}")
            raise
        finally:
