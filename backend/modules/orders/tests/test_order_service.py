...
from modules.investors.tests.conftest import create_fake_asset, create_fake_investor, asset_factory
...
def test_create_market_sell_insufficient_due_to_blocked(asset_factory):
    ...
    fake_market_order(investor=investor, ticker=instrument, is_buy=False, volume=Decimal("3"), save=True)

    svc = OrderService()
    order = svc.create_market(investor=investor, instrument=instrument, volume=Decimal("3"), is_buy=False)
...
def test_create_market_sell_success_when_enough_available(asset_factory):
    ...
    svc = OrderService()
    order = svc.create_market(investor=investor, instrument=instrument, volume=Decimal("2"), is_buy=False)
...
def test_create_limit_sell_respects_blocked_from_other_orders(asset_factory):
    ...
    fake_market_order(investor=investor, ticker=instrument, is_buy=False, volume=Decimal("5"), save=True)
    fake_limit_order(investor=investor, ticker=instrument, is_buy=False, volume=Decimal("3"), save=True)
