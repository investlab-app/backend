...
async def _send_ticker_data(layer, msg):
    await layer.group_send(
        'tickers_broadcast',
        {'type': 'broadcast.receive', 'data': msg}
    )
...
async def test_single_ticker():
    ...
    await _send_ticker_data(layer, {'AAPL': 'some_data', 'XYZ': 'other_data', 'ABC': 'more_data'})
...
async def test_multiple_tickers():
    ...
    await _assert_communicator_output(communicator, {'message': ['some_data', 'more_data']})
    await communicator.disconnect()