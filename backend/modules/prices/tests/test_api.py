...
def test_prices_view_success(api_client_auth, mock_yfinance_repository) -> None:
    url = reverse("prices")
    with container.prices_container.yfinance_repository.override(
...
def test_missing_param_returns_400(api_client_auth) -> None:
    url = reverse("prices")
...
def test_startdate_gt_enddate(api_client_auth) -> None:
    url = reverse("prices")
    response = api_client_auth.get(
        url,
        {
...
def test_invalid_interval(api_client_auth) -> None:
    url = reverse("prices")
    response = api_client_auth.get(
        url,
        {
